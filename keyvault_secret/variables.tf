...
variable "ephemeral_secret_content" {
  description = "Content of the ephemeral secret (value to store in Key Vault)"
  type        = string
  default     = ""
}

variable "ephemeral_secret_name" {
  description = "Name of the ephemeral secret in Key Vault"
  type        = string
  default     = ""
}

variable "ephemeral_secret_version" {
  description = "Version of the ephemeral secret (if applicable)"
  type        = number
  default     = 1
}

variable "ephemeral_secret_content_type" {
  description = "Content type of the ephemeral secret (e.g. text/plain)"
  type        = string
  default     = "text/plain"
}

variable "ephemeral_secret_tags" {
  description = "Tags to associate with the ephemeral secret"
  type        = map(string)
  default     = {}
}

variable "ephemeral_secret_not_before_date" {
  description = "Not before date for the ephemeral secret (ISO8601, optional)"
  type        = string
  default     = null
  validation {
    condition     = var.ephemeral_secret_not_before_date == null || can(timeadd(timestamp(), "1s"))
    error_message = "ephemeral_secret_not_before_date must be in valid ISO8601 format"
  }
}

variable "ephemeral_secret_expiration_date" {
  description = "Expiration date for the ephemeral secret (ISO8601, optional)"
  type        = string
  default     = null
  validation {
    condition     = var.ephemeral_secret_expiration_date == null || can(timeadd(timestamp(), "1s"))
    error_message = "ephemeral_secret_expiration_date must be in valid ISO8601 format"
  }
}
